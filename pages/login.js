...
const Login = () => {
  ...
  const handleSubmit = async (e)=>{
    ...
    setPassword('')
    if(responce.success){
      localStorage.setItem('myuser',JSON.stringify({token:responce.token,email:responce.email,name:responce.name,profile_image:responce.profile_image}))
  ...
  return (
    ...
    <div className="max-w-md w-full space-y-8">
      <div>
        ...
        <h2 className="mt-6 text-center text-3xl tracking-tight font-bold text-gray-900">Login</h2>
        <p className="mt-2 text-center text-sm text-gray-600">
          Or
          <Link href={'/signup'}><a href="#" className="font-medium text-cyan-600 hover:text-cyan-500"> Signup </a></Link>
        </p>
      </div>
      <form className="mt-8 space-y-6" onSubmit={handleSubmit} method="POST">
        ...
        <Link href={'/forgot'}><a href="#" className="font-medium text-cyan-600 hover:text-cyan-500"> Forgot your password? </a></Link>  
.          </div>
        </div>
  
        <div>
          <button type="submit" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-800 hover:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-800">
