...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (myuser && myuser.token) {
      setUser(myuser)
      getposts(myuser.token)
    }
  }, [router.query])

  const getposts = async (token)=>{
    let data = { token: token }
    let a = await fetch('http://127.0.0.1:8000/getposts/', {
      method: 'POST',
      headers: {
    ...
    let json_res = await a.json()
    setPosts(JSON.parse(JSON.stringify(json_res)))
  }
  ...
  return (<>
    ...
    <section className="mb-20 text-gray-600 body-font mx-auto">
            ...
            return <div key={item._id} className="xl:w-3/5 md:w-3/4 w-full lg:h-1/2 py-4 lg:pl-10 mx-auto">
              ...
              <Link passHref={true} key={item._id} href={`/post/${item.slug}`}>
                <div className="bg-gray-100 rounded-lg md:h-3/4 lg:h-1/2 cursor-pointer">
                  {item.image && <img className="h-full rounded w-full object-cover object-center md:mb-6" src={item.image.slice(6,)} alt="content" />}

                </div></Link>

              {/* like comment share buttons */}
              <div className='flex items-center space-x-4 py-0 px-2 lg:w-1/2 md:w-1/2'>
